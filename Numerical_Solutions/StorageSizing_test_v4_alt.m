...
for max_E1=fliplr(1:maxE_stepSize_E1:max_E_SIZE(1))
    ...
    E2_counter=length(5:maxE_stepSize_E2:max_E_SIZE(2))+1;
    for max_E2=fliplr(5:maxE_stepSize_E2:max_E_SIZE(2))
        ...
        RES_E1=2/(200*maxE_stepSize_E1);%*size1_mult);
        RES_E2=2/(maxE_stepSize_E2);%size2_mult);
        RES_L=3; %10/(2*size1_mult+2*size1_mult+2*E_MAX(2)); %1/(MAX_D_1+MAX_C_1+2*E_MAX(2)) %/(2*size1_mult+3*size2_mult);
        RES_U1=1; %/size1_mult;
        ...
        optVal_initE=ConvCosts((1-0)+1,(1-0)+1,round((-MIN_LOAD)*RES_L+1));
