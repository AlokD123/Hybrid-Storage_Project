...
%V3: UPDATED stage cost
  ...
  if(U1>MAX_DISCHARGE(1)||(L-U1)<-MAX_CHARGE(2)||U1<-MAX_CHARGE(1)||(L-U1)>MAX_DISCHARGE(2)) %If control is too high or low
    ...
    cost=(1/ALPHA_D(1)-1)*max(U1,0)/E_MAX(1)+(1/ALPHA_D(2)-1)*max(L-U1,0)/E_MAX(2)-(1-ALPHA_C(2))*min(0,L-U1)/E_MAX(2)-(1-ALPHA_C(1))*min(0,U1)/E_MAX(1)  +(1-E1/E_MAX(1))^2*(K(1)*(max(U1,0)/E_MAX(1))^2+K(2)*(min(0,U1)/E_MAX(1))^2) +nu*(E1/E_MAX(1))^2;
  end
end