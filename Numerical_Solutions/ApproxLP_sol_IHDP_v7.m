...
% v7: AUTOMATIC BASIS FUNCTION GENERATION (iterative), v4: initial design matrix
% setup by state aggregation
...
R=0; %MAXIMUM number of extra bases added by iteration (TOTAL MUST BE LESS THAN NUMBER OF FEASIBLE STATES)
...
%c_state=ones(size(Q,2),1);
  ...
  %Part 1: get initial Lagrange Multipliers
  %Run INITIAL optimization problem, to get INITIAL APPROXIMATE DUAL.
  cvx_begin
  ...
  figure
  hold on;
  plot(Phi*r_fit, '*'); %plot(Phi*r_fit,Phi*r_fit, '*');
  plot(cost, '*'); %plot(Phi*r_fit,cost, '*');
  xlabel('Approximate Cost');
  legend('Approximate Cost','Actual Cost');
  
  %Store approximation error for using k bases (2-NORM)
  approx_err=norm(cost-Phi*r_fit,2);
  %Store approximation
  approx=Phi*r_fit;
  
  %Loop to alternate between cost approximation and basis function
  %generation
  while k<=R %Add maximum possible number of bases
      %1) Get next basis vector automatically
      %Solve optimization to get vector...
      cvx_begin
        params.OptimalityTol = tolerance; %Set tolerance
        variable phi_k(size(Phi,1))
        dual variables d1
        dual variables d2
        dual variables d3
      ...
      %3) Solve approximate primal to see if cost approximation has
      %improved (and get next approximate dual)
      cvx_begin
        grbControl.LPMETHOD = 1; % Use dual simplex method
        params.OptimalityTol = tolerance; %Set tolerance
        variable r_fit(size(Phi,2))
  ...
  title(strcat('Testing Cost Approximation Using', num2str(R+10),'-Bases Fit'));
  hold off
  
  figure
  plot(approx_err/norm(cost,2));
  xlabel('Number of Extra Basis Vectors Added (In Addition to State Aggregation)');
  ylabel('2-Norm Error in Cost Approximation');
  title('Normalized Cost Approximation Error');
