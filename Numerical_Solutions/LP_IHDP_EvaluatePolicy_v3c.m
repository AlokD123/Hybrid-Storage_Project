...
global MAX_CHARGE; global MAX_LOAD; global MIN_LOAD;
...
for i=1:1
    ...
    seqL=[seqL,(1+normrnd(0.2,0.1,1,129)).*linspace(3.2,-2,129).*(1+0.5*sin(linspace(1,10,129)))];
end
%}
...
%seqL(seqL<0)=0;                 %To test w/ no R.B.
...
while t_ind_VI<NumIter
    ...
    while isempty(D1)||isempty(D2)||isempty(C2)||(D1+D2-C2-L)<-eps||(D1+D2-C2-L)>MAX_CHARGE(1)+eps %Infeasible if no optimal control OR C1 constraints violated
        if L<-epsilon
            L=L+DeltaL_min;
    ...
    while(nextE1>E_MAX(1)+eps||nextE1<E_MIN(1)-eps||nextE2>E_MAX(2)+eps||nextE2<E_MIN(2)-eps||(D1+D2-C2-L)<-eps||(D1+D2-C2-L)>MAX_CHARGE(1)+eps)
