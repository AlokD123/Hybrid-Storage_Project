%ALP solution of IHDP (Value Iteration) for Hybrid Storage **SIZING**
% With E_MAX input (optimization variables)
  ...
  for U1_Ind=1:(P0+P1-1)
                ...
                if(U1>E1 || U1<(E1-E_MAX(1)))
                    ...
                    for indL=1:(MAX_DISCHARGE(2)+MAX_CHARGE(2)-MIN_LOAD+1)
                        ...
                        if(nextE_Ind~=-1) %If this state leads to a feasible next state...
                            gVec_p(indCount)=CtrlCost_Modified_v2(U1,L); %Cost of stage is given by CtrlCost
      ...
      for i=1:size(nextE_Ind_Vect_p,1) %For each next state...
         ...
         if round(nextE_Ind_Vect_p(i))~=nextE_Ind_Vect_p(i) %ONLY if off grid..
             ...
             numLoads_OffGrd_p=[numLoads_OffGrd_p;nnz(abs(nextE_Ind_L_Vect_p-nextE_Ind_Vect_p(i))<epsilon2)];
