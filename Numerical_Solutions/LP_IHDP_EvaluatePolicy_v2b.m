...
for i=1:1
    seqL=[0.5*(1+normrnd(0.2,0.2,1,140)).*(1+0.1*sin(linspace(1,50,140))).*(1-0.2*sin(linspace(1,10,140)))];
...
while t_ind_VI<=NumIter
    ...
    %{
    %Round next state to nearest int %<---------------------------------------------------------------------------------------- ADDED ROUNDING (to remove!)--------------------
    nextE1=round(nextE1);
    nextE2=round(nextE2);
    %}
