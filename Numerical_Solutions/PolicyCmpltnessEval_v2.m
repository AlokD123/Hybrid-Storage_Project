%Get:
%   1) maximum q-value for each state-action, used to determine optimal control
    ...
    for j=1:N2
        for k=1:size(feasStates,3)
            if(feasStates(i,j,k)==1)
               MaxQValues=[MaxQValues;i,j,k,max(qValsMtx(i,j,k,:))];
               NumOptCtrls_1=[NumOptCtrls_1;i,j,k,nnz(abs(qValsMtx(i,j,k,:)-max(qValsMtx(i,j,k,:)))<epsilon5)];
               l=l+1;
